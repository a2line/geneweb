...
let rec print_descend_upto conf base cnt cnt_sp max_cnt ini_p ini_br lev children =
    ...
    begin
      ...
      let (cnt, cnt_sp) =
				List.fold_left
								 ...
								 (* the function children_of returns *all* the children of ip *)
									 ...
									 List.fold_left
										 (fun (cnt, cnt_sp) ifam ->
												...
												print_descend_upto conf base cnt cnt_sp max_cnt ini_p ini_br (lev - 1) children;
										 )
					...
					(cnt, cnt_sp) children
      in
      let _ = Output.print_string conf "</ul>\n" in
      (cnt, cnt_sp)
    end
  else
    (cnt, cnt_sp)

let print_cousins_side_of conf base cnt cnt_sp max_cnt a ini_p ini_br lev1 lev2 =
...
let print_cousins conf base p lev1 lev2 =
  ...
  let (cnt, cnt_sp) = print_cousins_lev conf base max_cnt p lev1 lev2 in
  Output.print_string conf "<div>\n";
