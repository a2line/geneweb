...
let my_uppercase s =
  String.uppercase_ascii (Name.lower s)
  ...
  Gwdb.Collection.iteri
    (fun i p ->
      ...
      let d_date =
        ...
        | _ -> None
      in
      let sn = p_surname base p |> Name.lower |> my_uppercase in
      let fn =
        ...
        | first :: _ -> (
            ...
            with Not_found -> fn)
      in
      let fn = Name.lower fn |> my_uppercase in
          ...
          if bd.year >= !min_birth_year && check = true then (
            ...
            | Some dd ->
                safe_printf "%s|%s|%d|%02d|%02d|%04d|%s|%02d|%02d|%04d|%s|%s\n"
                  ...
                  (if bd.prec = Sure || bd.prec = About || bd.prec = Maybe then bd.year else 0)
                  b_place
                  (if dd.prec = Sure || dd.prec = Maybe then dd.day else 0)
                  (if dd.prec = Sure || dd.prec = Maybe then dd.month else 0)
                  (if dd.prec = Sure || dd.prec = About || dd.prec = Maybe then dd.year else 0)
                  d_place key
            | None ->
